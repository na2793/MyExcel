...
public class Sheet {
    ...
    public Cell createCell(int column, int row, String value, int type) {
        ...
        if (column <= 0 || row <= 0 || column > DEFAULT_SHEET_MAX_COLUMN || row > DEFAULT_SHEET_MAX_ROW) {
            throw new CellOutOfSheetBoundException("cell (" + column + ", " + row + ") can't be in " + mName + ". (maxColumn = " + DEFAULT_SHEET_MAX_COLUMN + ", maxRow = " + DEFAULT_SHEET_MAX_ROW + ")");
        }
        ...
        if (!exist) {
            if (DEFAULT_SHEET_MAX_ROW % SHEET_BLOCK_ROW != 0) {
                int maxBlockColumn = (int) Math.ceil((float) DEFAULT_SHEET_MAX_COLUMN / (float) SHEET_BLOCK_COLUMN);
                int maxBlockRow = (int) Math.ceil((float) DEFAULT_SHEET_MAX_ROW / (float) SHEET_BLOCK_ROW);

                // 배열을 찾습니다.
                if (blockNum == maxBlockColumn * maxBlockRow) {
                    cells = new Cell[DEFAULT_SHEET_MAX_ROW % SHEET_BLOCK_ROW][DEFAULT_SHEET_MAX_COLUMN % SHEET_BLOCK_COLUMN];
                } else if ((maxBlockColumn - 1) * maxBlockRow < blockNum && blockNum < maxBlockColumn * maxBlockRow) {
                    cells = new Cell[DEFAULT_SHEET_MAX_ROW % SHEET_BLOCK_ROW][SHEET_BLOCK_COLUMN];
                } else if (blockNum % maxBlockColumn == 0) {
                    cells = new Cell[SHEET_BLOCK_ROW][DEFAULT_SHEET_MAX_COLUMN % SHEET_BLOCK_COLUMN];
                } else {
    ...
    private int getCellBlockNum(int column, int row) {
        ...
        int lastBlockColumn = DEFAULT_SHEET_MAX_COLUMN % SHEET_BLOCK_COLUMN;
        int blockNum;
        ...
        // 쭉 펴서
        blockNum = (int) ((float) row / (float) SHEET_BLOCK_ROW) * DEFAULT_SHEET_MAX_COLUMN + column;

        // 마지막 column에 부족한 만큼 더하고
        blockNum += (blockNum / DEFAULT_SHEET_MAX_COLUMN) * (SHEET_BLOCK_COLUMN - lastBlockColumn);
